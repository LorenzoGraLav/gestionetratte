...
public class TrattaDTO {
	...
	public TrattaDTO(String codice, String descrizione, LocalDate data, LocalTime oraDecollo, LocalTime oraAtterraggio,
			Stato stato, AirbusDTO airbus) {
		super();
		this.codice = codice;
		this.descrizione = descrizione;
		this.data = data;
	...
	public static Set<TrattaDTO> createTratteDTOSetFromModelSet(Set<Tratta> modelListInput, boolean includeAirbus) {
		return modelListInput.stream().map(trattaEntity -> {
			return TrattaDTO.buildTrattaDTOFromModel(trattaEntity, includeAirbus);
